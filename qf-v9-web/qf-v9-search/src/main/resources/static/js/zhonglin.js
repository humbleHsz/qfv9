...
//轮播图 END
	...
	$(".favorites .fa-left li").hover(function() {
		$(this).addClass('current').siblings().removeClass('current');
		$(".fa-right .fa-r-con .fa-con-uls").eq($(this).list()).show().siblings().hide();
	});
	
	$("[uls1] li").click(function(event) {
		$(this).addClass('current').siblings().removeClass('current');
		var us1 = $(this).parent("[uls1]").attr("uls1");
		$("[uls2="+us1+"] .ul1").eq($(this).list()).show().siblings().hide();
	});
	...
	$("[qie-xiao] li").hover(function() {
		$(this).addClass('current').siblings().removeClass('current');
		$("[qie-da] li").eq($(this).list()).show().siblings().hide();
	});
	...
	$("[shuxing1] li").click(function(event) {
		$(this).addClass('current').siblings().removeClass('current');
		$("[shuxingsw1] ul").eq($(this).list()).show().siblings().hide();
	});
	$("[shuxing2] li").click(function(event) {
		$(this).addClass('current').siblings().removeClass('current');
		$("[shuxingsw2] ul").eq($(this).list()).show().siblings().hide();
	});
	$("[shuxing3] li").click(function(event) {
		$(this).addClass('current').siblings().removeClass('current');
		$("[shuxingsw3] ul").eq($(this).list()).show().siblings().hide();
	});
	/*搜索列表页(有品牌) js*/
